...
from ...constants import VOCALOID_COLOR
...
def build_vocaloid_embed(character, image_detail):
    ...
    if (image_detail is None):
        embed = Embed(None, '*Could not get any images, please try again later.*', color = VOCALOID_COLOR)
    
    else:
        embed = Embed(
            VOCALOID_CHARACTER_TO_FULL_NAME[character], url = image_detail.url, color = VOCALOID_COLOR
        ).add_image(
            image_detail.url,
        )
