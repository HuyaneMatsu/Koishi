...
from ...constants import TOUHOU_COLOR
from ...helpers import add_provider
from ...touhou import get_familiar_touhou_matches
...
def build_touhou_character_embed(touhou_character, image_detail):
    ...
    if (image_detail is None):
        embed = Embed(None, '*Could not get any images, please try again later.*', color = TOUHOU_COLOR)
    
    else:
        embed = Embed(
            touhou_character.name, url = image_detail.url, color = TOUHOU_COLOR
        ).add_image(
            image_detail.url,
        )
...
def build_no_match_embed(name):
    ...
    embed : ``Embed`
    """
    embed = Embed('No match', color = TOUHOU_COLOR)
