...
from hata import alchemy_incendiary, sleep, Task, Embed, eventlist, Color, YTAudio, DownloadError, LocalAudio, \
    Permission, KOKORO, ChannelBase, ChannelVoice
from hata.ext.commands import Command, checks, Pagination

import pers_data

VOICE_COLOR = Color.from_rgb(235, 52, 207)
VOICE_COMMANDS = eventlist(type_=Command, category='VOICE')
...
@VOICE_COMMANDS.from_class
class move:
    async def command(client, message, channel:ChannelVoice = None):
        while True:
            if channel is None:
                guild = message.guild
                if guild is None:
...
class queue:
    async def command(client, message):
        ...
        page = Embed(title, color=VOICE_COLOR)
        pages = [page]
        while True:
            if voice_client is None:
