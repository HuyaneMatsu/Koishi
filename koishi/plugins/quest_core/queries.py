__all__ = (
    'add_linked_quest', 'delete_linked_quest', 'get_linked_quest_listing', 'update_linked_quest'
)
...
def _pop_completed_linked_quests(linked_quest_listing):
    ...
    linked_quest_listing : ``None | list<LinkedQuest>``
        Linked quests to pop from.
    
    Returns
    -------
    linked_quest_listing : ``None | list<LinkedQuest>``
        Remaining linked quests.
    """
    if (linked_quest_listing is None):
        return None
    
    quest_batch_id = get_current_batch_id()
    ...
    for linked_quest in linked_quest_listing:
        ...
        if linked_quests_to_pop is None:
            linked_quests_to_pop = []
        
        linked_quests_to_pop.append(linked_quest)
    
    if (linked_quests_to_pop is None):
        return linked_quest_listing
    
    if len(linked_quests_to_pop) == linked_quest_listing:
        linked_quest_listing = None
    else:
        for linked_quest in linked_quests_to_pop:
            try:
                linked_quest_listing.remove(linked_quest)
            except ValueError:
                pass
    
    Task(KOKORO, query_delete_linked_quests([linked_quest.entry_id for linked_quest in linked_quests_to_pop]))
    return linked_quest_listing
...
async def get_linked_quest_listing(user_id):
    ...
    """
    try:
        linked_quest_listing = LINKED_QUEST_LISTING_CACHE[user_id]
    except KeyError:
        pass
    else:
...
def _linked_quest_listing_query_done_callback(key, waiters, task):
    ...
    """
    try:
        linked_quest_listing = task.get_result()
        ...
        for waiter in waiters:
            waiter.set_result_if_pending(linked_quest_listing)
...
async def query_delete_linked_quests(entry_ids):
    """
    Deletes the linked quests from the database.
    ...
    entry_ids : `list<entry_ids>`
        The entry's identifiers to delete.
    """
    async with DB_ENGINE.connect() as connector:
        await connector.execute(
            LINKED_QUEST_TABLE.delete().where(
                linked_quest_model.id.in_(entry_ids),
            ),
        )

if DB_ENGINE is None:
    @copy_docs(query_delete_linked_quests)
    async def query_delete_linked_quests(entry_ids):
        return
...
async def update_linked_quest(linked_quest):
    """
    Updates the linked quest in the database.
    ...
    ----------
    linked_quest : ``LinkedQuest``
        Linked quest to update.
    """
    if linked_quest.entry_id == 0:
        return
    
    await query_update_linked_quest(linked_quest)


async def query_update_linked_quest(linked_quest):
    """
    Executes the update query for updating a linked quest in the database.
...
if DB_ENGINE is None:
    @copy_docs(query_update_linked_quest)
    async def query_update_linked_quest(linked_quest):
        return
