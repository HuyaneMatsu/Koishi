...
from .embed_builders import (
    ...
    build_failure_embed_they_already_have_master, build_failure_embed_they_already_have_waifu,
    build_failure_embed_you_already_have_mama, build_failure_embed_you_already_have_master,
...
from .relationship_types import RELATIONSHIP_TYPE_MAMA, RELATIONSHIP_TYPE_MASTER, RELATIONSHIP_TYPE_WAIFU


def check_self_propose(source_user, target_user):
    """
    Checks whether source user is the same as teh target user.
...
async def async_check_target_already_has_master(
    relationship_type, target_relationship_listing, checked_at_creation, source_user, target_user, guild_id
):
    """
    Checks whether the target user already has a master.
    ...
    InteractionAbortedError
    """
    if relationship_type != RELATIONSHIP_TYPE_MASTER:
        return
    
    relationship = select_relationship(target_user.id, RELATIONSHIP_TYPE_MASTER, target_relationship_listing)
    if relationship is None:
        return
    
    master_id = relationship.source_user_id
    if master_id == target_user.id:
        master_id = relationship.target_user_id
    master = await get_user(master_id)
    
    if checked_at_creation:
        user = target_user
        embed_builder = build_failure_embed_they_already_have_master
    else:
        user = source_user
        embed_builder = build_failure_embed_you_already_have_master
    
    abort(
        embed = embed_builder(master, user, guild_id),
    )
