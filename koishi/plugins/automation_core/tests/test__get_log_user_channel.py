...
from ..operations import get_log_user_channel
from ..constants import AUTOMATION_CONFIGURATIONS


def _iter_options():
    guild_id = 202405300052
    ...
    guild_id = 202405300053
    automation_configuration = AutomationConfiguration(guild_id)
    automation_configuration.log_user_enabled = False
    ...
    guild_id = 202405300055
    channel_id = 202405300056
    automation_configuration = AutomationConfiguration(guild_id)
    automation_configuration.log_user_enabled = False
    automation_configuration.log_user_channel_id = channel_id
    ...
    guild_id = 202405300057
    channel_id = 202405300058
    automation_configuration = AutomationConfiguration(guild_id)
    automation_configuration.log_user_enabled = True
    automation_configuration.log_user_channel_id = channel_id
    ...
    guild_id = 202405300059
    channel_id = 202405300060
    ...
    guild_id = 202405300061
    channel_id = 202405300062
    channel = Channel.precreate(channel_id)
    automation_configuration = AutomationConfiguration(guild_id)
    automation_configuration.log_user_enabled = True
    automation_configuration.log_user_channel_id = channel_id
...
def test__get_log_user_channel(automation_configuration, guild_id, extras):
    """
    Tests whether ``get_log_user_channel`` works as intended.
    ...
    try:
        ...
        output = get_log_user_channel(guild_id)
        vampytest.assert_instance(output, Channel, nullable = True)
        
        return(
            output,
            (0 if automation_configuration is None else automation_configuration.log_user_channel_id),
        )
