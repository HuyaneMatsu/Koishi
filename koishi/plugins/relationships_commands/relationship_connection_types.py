__all__ = ()

from ..relationships_core import (
    RELATIONSHIP_TYPE_AUNTIE, RELATIONSHIP_TYPE_CO_WORKER, RELATIONSHIP_TYPE_DAUGHTER, RELATIONSHIP_TYPE_GRANDDAUGHTER,
    RELATIONSHIP_TYPE_GRANNY, RELATIONSHIP_TYPE_MAID, RELATIONSHIP_TYPE_MAMA, RELATIONSHIP_TYPE_MISTRESS,
    RELATIONSHIP_TYPE_NANNY, RELATIONSHIP_TYPE_NIECE, RELATIONSHIP_TYPE_SISTER_BIG, RELATIONSHIP_TYPE_SISTER_LIL,
    RELATIONSHIP_TYPE_SISTER_RELATIVE, RELATIONSHIP_TYPE_TEA_FRIEND, RELATIONSHIP_TYPE_WAIFU,
    RELATIONSHIP_TYPE_YOUNG_MISTRESS
)

RELATIONSHIP_CONNECTION_TYPE_NAME_DEFAULT = 'pudding'

RELATIONSHIP_CONNECTION_TYPE_NONE = 0
RELATIONSHIP_CONNECTION_TYPE_HALF = 1
RELATIONSHIP_CONNECTION_TYPE_STEP = 2
RELATIONSHIP_CONNECTION_TYPE_IN_LAW = 3


RELATIONSHIP_CONNECTION_NAMES = {
    RELATIONSHIP_CONNECTION_TYPE_NONE : None,
    RELATIONSHIP_CONNECTION_TYPE_HALF : 'half',
    RELATIONSHIP_CONNECTION_TYPE_STEP : 'step',
    RELATIONSHIP_CONNECTION_TYPE_IN_LAW : 'in law',
}


def get_relationship_connection_type_name(relationship_connection_type):
    """
    Gets the relationship connection's name.
    
    Parameters
    ----------
    relationship_connection_type : `int`
        Relationship connection type.
    
    Returns
    -------
    relationship_connection_name : `None | str`
    """
    return RELATIONSHIP_CONNECTION_NAMES.get(relationship_connection_type, RELATIONSHIP_CONNECTION_TYPE_NAME_DEFAULT)


RELATION_TYPE_EXTEND_RESOLUTION = {
    # from waifu
    (
        RELATIONSHIP_TYPE_WAIFU,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ),
    (
        RELATIONSHIP_TYPE_WAIFU,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ),
    (
        RELATIONSHIP_TYPE_WAIFU,
        RELATIONSHIP_TYPE_MAMA,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_MAMA,
    ),
    (
        RELATIONSHIP_TYPE_WAIFU,
        RELATIONSHIP_TYPE_MAID,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_MAID,
    ),
    (
        RELATIONSHIP_TYPE_WAIFU,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_DAUGHTER,
    ),
    
    # from little sister
    (
        RELATIONSHIP_TYPE_SISTER_LIL,
        RELATIONSHIP_TYPE_WAIFU,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_LIL,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_LIL,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_SISTER_RELATIVE,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_LIL,
        RELATIONSHIP_TYPE_MAMA,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_STEP,
        RELATIONSHIP_TYPE_MAMA,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_LIL,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_NIECE,
    ),
    
    # from big sister
    (
        RELATIONSHIP_TYPE_SISTER_BIG,
        RELATIONSHIP_TYPE_WAIFU,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_BIG,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_SISTER_RELATIVE,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_BIG,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_BIG,
        RELATIONSHIP_TYPE_MAMA,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_STEP,
        RELATIONSHIP_TYPE_MAMA,
    ),
    (
        RELATIONSHIP_TYPE_SISTER_BIG,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_NIECE,
    ),
    
    # from mama
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_WAIFU,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_MAMA,
    ),
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_AUNTIE,
    ),
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_AUNTIE,
    ),
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_MAMA,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_GRANNY,
    ),
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_SISTER_RELATIVE,
    ),
    (
        RELATIONSHIP_TYPE_MAMA,
        RELATIONSHIP_TYPE_MAID,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_NANNY,
    ),
    
    # from daughter
    (
        RELATIONSHIP_TYPE_DAUGHTER,
        RELATIONSHIP_TYPE_WAIFU,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_DAUGHTER,
    ),
    (
        RELATIONSHIP_TYPE_DAUGHTER,
        RELATIONSHIP_TYPE_SISTER_BIG,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_DAUGHTER,
    ),
    (
        RELATIONSHIP_TYPE_DAUGHTER,
        RELATIONSHIP_TYPE_SISTER_LIL,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_HALF,
        RELATIONSHIP_TYPE_DAUGHTER,
    ),
    (
        RELATIONSHIP_TYPE_DAUGHTER,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_GRANDDAUGHTER,
    ),
    (
        RELATIONSHIP_TYPE_DAUGHTER,
        RELATIONSHIP_TYPE_MISTRESS,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_TEA_FRIEND,
    ),
    
    # from mistress
    (
        RELATIONSHIP_TYPE_MISTRESS,
        RELATIONSHIP_TYPE_WAIFU,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_IN_LAW,
        RELATIONSHIP_TYPE_MISTRESS,
    ),
    (
        RELATIONSHIP_TYPE_MISTRESS,
        RELATIONSHIP_TYPE_DAUGHTER,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_YOUNG_MISTRESS,
    ),
    (
        RELATIONSHIP_TYPE_MISTRESS,
        RELATIONSHIP_TYPE_MAID,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_CO_WORKER,
    ),
    
    # shared through maid
    (
        RELATIONSHIP_TYPE_MAID,
        RELATIONSHIP_TYPE_MAMA,
    ) : (
        RELATIONSHIP_CONNECTION_TYPE_NONE,
        RELATIONSHIP_TYPE_TEA_FRIEND,
    ),
}
