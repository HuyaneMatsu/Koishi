...
from ..helpers import PERMISSION_MASK_MESSAGING_DEFAULT, sum_votes
...
async def test__sum_votes():
    """
    Tests whether ``sum_votes`` works as intended.
    """
    client = Client(
        'token_202301160015',
        client_id = 202301160016,
        api = TestDiscordApiClient(True, 'token_202301160015'),
        
    )
    try:
        guild = Guild.precreate(202301160017)
        channel = Channel.precreate(202301160018, guild_id = guild.id, channel_type = ChannelType.guild_text)
        user_0 = User.precreate(202301160019, bot = False)
        user_0.guild_profiles[guild.id] = GuildProfile()
        user_1 = User.precreate(202301160022)
        ...
        emoji = Emoji.precreate(202301160020)
