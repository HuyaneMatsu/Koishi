...
from hata import Embed, Color, elapsed_time
...
async def threads_description(command_context):
    return Embed(
        'threads',
        (
...
async def threads(client, message):
...
if PYPY:
    ...
    @COMMAND_CLIENT.commands(
        aliases = ['gc', 'gc-info'],
        category = 'STATS',
        description = gc_stats_description,
    )
    async def gc_stats(client, message):
        stats = get_gc_stats()
...
if (CpuUsage is not None):
    async def system_stats_description(command_context):
        return Embed(
            'system-stats',
            (
    ...
    @COMMAND_CLIENT.commands(
        ...
        description = system_stats_description,
        category = 'STATS',
    )
