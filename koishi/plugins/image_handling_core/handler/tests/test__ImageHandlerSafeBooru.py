...
from ..safe_booru import ImageHandlerSafeBooru, PROVIDER_SAFE_BOORU
...
def _assert_fields_set(image_handler):
    ...
    Parameters
    ----------
    image_handler : ``ImageHandlerSafeBooru``
        The image handler to check.
    """
    vampytest.assert_instance(image_handler, ImageHandlerSafeBooru)
...
def test__ImageHandlerSafeBooru__new():
    """
    Asserts whether ``ImageHandlerSafeBooru.__new__`` works as intended.
    """
    ...
    image_handler = ImageHandlerSafeBooru(tags_required, tags_banned, tags_requested, random_order)
...
async def test__ImageHandlerSafeBooru__cg_get_image(keyword_parameters, attributes_to_set, request_return):
    """
    Tests whether ``ImageHandlerSafeBooru.cg_get_image`` works as intended.
    ...
    image_handler = ImageHandlerSafeBooru(**keyword_parameters)
    
    if (attributes_to_set is not None):
        for item in attributes_to_set:
            setattr(image_handler, *item)
    
    original_request = ImageHandlerSafeBooru._request
    ImageHandlerSafeBooru._request = patched_request
    ...
    try:
        ...
        ImageHandlerSafeBooru._request = original_request
...
def test__ImageHandlerSafeBooru__eq(keyword_parameters_0, keyword_parameters_1):
    """
    Tests whether ``ImageHandlerSafeBooru.__eq__`` works as intended.
    ...
    output : `bool`
    """
    image_handler_0 = ImageHandlerSafeBooru(**keyword_parameters_0)
    image_handler_1 = ImageHandlerSafeBooru(**keyword_parameters_1)
