...
async def test__notify_user__success():
    ...
    Case: Success.
    """
    client_id = 202604280003
    channel_id = 202604280004
    user_id = 202604280005
    message_id = 202604280006
    entry_id = 23
    connector = None
    ...
    async def message_create(self, input_channel, **keyword_parameters):
        ...
        vampytest.assert_eq({*keyword_parameters}, {'allowed_mentions', 'components'})
        return message
    
    
    async def set_entry_as_notified_with_connector(input_connector, input_entry_id, mark_seller):
        nonlocal connector
        nonlocal entry_id
    ...
    try:
        output = await mocked((entry_id, 0, user_id, 0, 0, 0, 0, 0, user_id, client_id), connector)
