__all__ = ('TOUHOU_ACTION_LICK',)

from ...image_handling_core import ImageDetail
from ...touhou_core import (
    CHEN, CHIRUNO, FUJIWARA_NO_MOKOU, HAKUREI_REIMU, HOSHIGUMA_YUUGI, INABA_TEWI, INUBASHIRI_MOMIJI, IZAYOI_SAKUYA,
    KAENBYOU_RIN, KAMISHIRASAWA_KEINE, KAZAMI_YUUKA, KIRISAME_MARISA, KOCHIYA_SANAE, KOMEIJI_KOISHI, KOMEIJI_SATORI,
    KONPAKU_YOUMU, LETTY_WHITEROCK, MARGATROID_ALICE, MIZUHASHI_PARSEE, REISEN_UDONGEIN_INABA, RUMIA, SAIGYOUJI_YUYUKO,
    SCARLET_FLANDRE, SHAMEIMARU_AYA, TATARA_KOGASA, YAKUMO_RAN, YAKUMO_YUKARI
)
...
TOUHOU_ACTION_LICK = [
    ...
    ).with_source(KIRISAME_MARISA).with_target(MARGATROID_ALICE),
    ImageDetail(
    ...
    ).with_source(IZAYOI_SAKUYA).with_target(MARGATROID_ALICE),
    ImageDetail(
    ...
    ).with_source(YAKUMO_YUKARI).with_target(KIRISAME_MARISA),
    ImageDetail(
        'https://cdn.discordapp.com/attachments/568837922288173058/1220463488355532881/orin-satori-lick-0000.png',
    ).with_source(KAENBYOU_RIN).with_target(KOMEIJI_SATORI),
    ImageDetail(
    ...
    ).with_source(YAKUMO_RAN).with_target(YAKUMO_YUKARI),
    ImageDetail(
    ...
    ).with_source(RUMIA).with_target(KONPAKU_YOUMU),
    ImageDetail(
    ...
    ).with_source(SAIGYOUJI_YUYUKO).with_target(YAKUMO_YUKARI),
]
