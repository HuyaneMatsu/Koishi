...
from ..stats_core import get_stat_value_for_index, get_stats, set_stat_value_for_index
from ..stats_upgrade_core import (
...
async def stat_upgrade_invoke_self_question(client, event, stat_index):
    ...
    stat_value_after = get_stat_value_for_index(stats, stat_index) + 1
...
async def stat_upgrade_invoke_other_question(client, event, target_user, relationship_to_deepen, stat_index):
    ...
    stats = await get_stats(target_user.id)
...
async def stat_upgrade_confirm_self(event, stat_index):
    ...
    set_stat_value_for_index(stats, stat_index, stat_value_after)
    await stats.save()
...
async def stat_upgrade_confirm_other(client, event, target_user_id, stat_index):
    ...
    stats = await get_stats(target_user_id)
