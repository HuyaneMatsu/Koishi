...
from ..utils import autocomplete_user_settings_preferred_client


async def test__autocomplete_user_settings_preferred_client__none():
    """
    Tests whether ``autocomplete_user_settings_preferred_client`` works as intended.
    ...
    mocked = vampytest.mock_globals(autocomplete_user_settings_preferred_client, 2, FEATURE_CLIENTS = FEATURE_CLIENTS)
    
    try:
        output = await mocked(event, None)
        vampytest.assert_instance(output, list)
        vampytest.assert_eq(output, [PREFERRED_CLIENT_NAME_DEFAULT, client_1_name, client_0_name])
...
async def test__autocomplete_user_settings_preferred_client__value_excluding_default():
    """
    Tests whether ``autocomplete_user_settings_preferred_client`` works as intended.
...
async def test__autocomplete_user_settings_preferred_client__value_including_default():
    """
    Tests whether ``autocomplete_user_settings_preferred_client`` works as intended.
    ...
    try:
        ...
        vampytest.assert_instance(output, list)
        vampytest.assert_eq(output, [PREFERRED_CLIENT_NAME_DEFAULT])
