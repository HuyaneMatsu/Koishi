...
from ..user_stats_core import get_user_stat_name_full_for_index
...
def build_failure_embed_insufficient_available_balance_self(
    ...
    return Embed(
        ...
        (
            f'You cannot upgrade your {get_user_stat_name_full_for_index(stat_index)} to {stat_value_after} '
...
def build_failure_embed_insufficient_available_balance_other(
    ...
    return Embed(
        'Insufficient available balance',
        (
            f'You cannot upgrade {user.name_at(guild_id)}\'s {get_user_stat_name_full_for_index(stat_index)} to '
...
def build_question_embed_purchase_confirmation_self(required_balance, stat_index, stat_value_after):
    ...
    return Embed(
        'Stat upgrade',
        (
            f'Are you sure to upgrade your {get_user_stat_name_full_for_index(stat_index)} to {stat_value_after} for '
...
def build_question_embed_purchase_confirmation_other(required_balance, stat_index, stat_value_after, user, guild_id):
    ...
    return Embed(
        'Stat upgrade',
        (
            f'Are you sure to upgrade {user.name_at(guild_id)}\'s {get_user_stat_name_full_for_index(stat_index)} to '
...
def build_success_embed_purchase_completed_self(balance_before, required_balance, stat_index, stat_value_after):
    ...
    embed = Embed(
        'Purchase successful',
        f'You upgraded your {get_user_stat_name_full_for_index(stat_index)} to {stat_value_after}.',
    )
...
def build_success_embed_purchase_completed_other(
    ...
    embed = Embed(
        'Purchase successful',
        f'You upgraded {user.name_at(guild_id)}\'s {get_user_stat_name_full_for_index(stat_index)} to {stat_value_after}.',
    )
...
def build_notification_embed_other(stat_index, stat_value_after, source_user, guild_id):
    ...
    return Embed(
        'Love is in the air',
        (
            f'{source_user.name_at(guild_id)} upgraded your {get_user_stat_name_full_for_index(stat_index)} to '
            f'{stat_value_after}.'
        ),
    )
