...
def _iter_options__can_activate():
    ...
    stage = _build_default_stage()
    stage.map[17] = BIT_MASK_FLOOR | BIT_MASK_BOX
    stage.map[10] = BIT_MASK_FLOOR | BIT_MASK_BOX
...
def test__SKILL_JUMP__can_activate(stage):
    """
    Tests whether ``SKILL_JUMP.can_activate`` works as intended.
    ...
    output = SKILL_JUMP.can_activate(game_state)
...
def test__SKILL_JUMP__get_directions(stage):
    """
    Tests whether ``SKILL_JUMP.get_directions`` works as intended.
    ...
    """
    game_state = GameState(stage, -1)
    output = SKILL_JUMP.get_directions(game_state)
...
def test__SKILL_JUMP__use(stage, step, align):
    """
    Tests whether ``SKILL_JUMP.use`` works as intended.
    ...
    """
    game_state = GameState(stage, -1)
    output = SKILL_JUMP.use(game_state, step, align)
    vampytest.assert_instance(output, bool)
