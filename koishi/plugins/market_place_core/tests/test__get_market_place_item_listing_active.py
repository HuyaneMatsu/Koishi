...
from ..queries import get_market_place_item_listing_active


def _iter_options():
    now = DateTime(2016, 5, 14, tzinfo = TimeZone.utc)
    
    user_id = 202512260010
    ...
    yield (
        ...
        (
            [
                market_place_item_00,
                market_place_item_01,
                market_place_item_02,
                market_place_item_03,
                market_place_item_04,
                market_place_item_05,
                market_place_item_06,
                market_place_item_07,
                market_place_item_08,
            ],
            False,
        ),
...
async def test__get_market_place_item_listing_active(
    item_id, item_flags, now, page_index, page_size, market_place_items_patched, market_place_items_cache_patched
):
    """
    Tests whether ``get_market_place_item_listing_active`` works as intended.
    ...
    output = await mocked(item_id, item_flags, now, page_index, page_size)
