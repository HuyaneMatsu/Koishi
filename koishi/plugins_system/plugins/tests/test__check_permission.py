...
def test__check_permission__no_role():
    ...
    Case: no role.
    """
    user = User.precreate(202409140001)
    event = InteractionEvent.precreate(202409140000, user = user)
...
def test__check_permission__has_role():
    ...
    Case: has role.
    """
    user = User.precreate(202409140003)
    user.guild_profiles[ROLE__SUPPORT__ADMIN.guild_id] = GuildProfile(
        role_ids = [ROLE__SUPPORT__ADMIN.id], 
    )
    
    event = InteractionEvent.precreate(202409140002, user = user)
