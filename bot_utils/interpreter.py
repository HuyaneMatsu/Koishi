...
class InterpreterInputter:
    ...
    def __call__(self, promt=None):
        ...
        with KOKORO.enter():
            task = Task(Pagination(client, channel, pages, timeout=300.), KOKORO)
    ...
    async def __call__(self,client,message,content):
        ...
        async with self.lock:
            ...
            try:
                ...
                try:
                    await KOKORO.run_in_executor(alchemy_incendiary(exec, (code_object, self.locals),),)
                except BaseException as err:
                    await KOKORO.render_exc_async(err,file=printer)
